...
def run_evaluation(dataset_path: Path, document_path: Path) -> list[dict]:
    ...
    for q in dataset["questions"]:
        ...
        try:
            response = ask_with_retry(q["question"])
        except Exception as e:
            print(f"❌ {q['id']}: échec après retries ({type(e).__name__}), on passe")
            response = {"answer": "[ERREUR API]", "sources": results}
...
def ask_with_retry(question: str, max_retries: int = 3):
    """Appelle ask() avec retry en cas de rate limit."""
    for attempt in range(max_retries):
        try:
            return ask(question)
        except Exception as e:
            if any(code in str(e) for code in ["RESOURCE_EXHAUSTED", "429", "503", "UNAVAILABLE"]):
                wait = 30 * (attempt + 1)
                print(f"⏳ Quota atteint, pause de {wait}s...")
                time.sleep(wait)
            else:
                raise
    raise RuntimeError("Quota toujours dépassé après plusieurs tentatives")